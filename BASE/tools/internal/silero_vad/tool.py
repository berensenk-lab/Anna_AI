...
class SileroVADTool(InternalToolInterface):
    ...
    def _on_speech_detected(self, audio_data: np.ndarray, sample_rate: int):
        ...
        try:
            if self._logger:
                self._logger.system(f"[Silero VAD] Transcribing {len(audio_data)} samples...")
            ...
            if self._logger:
                self._logger.system(f"[Silero VAD] Transcription result: '{text}' ({elapsed:.2f}s)")
            
            if text and len(text) >= 3:
                try:
                    self._text_queue.put_nowait(text)
                    if self._logger:
                        self._logger.system(f"[Silero VAD] Added to text queue: '{text}'")
                except queue.Full:
                    if self._logger:
                        self._logger.warning("[Silero VAD] Text queue full, replacing oldest")
                    try:
                ...
                if self._logger:
                    self._logger.system(f"[Silero VAD] Text too short or empty, ignored")
    ...
    def _transcribe_whisper(self, audio_data: np.ndarray, sample_rate: int) -> str:
        ...
        try:
            if self._logger:
                self._logger.system(f"[Silero VAD] Running Whisper on {len(audio_data)} samples...")
            ...
            for segment in segments:
                ...
                if segment_text:
                    ...
                    if self._logger:
                        self._logger.system(
                            f"[Silero VAD] Segment: '{segment_text}' "
                            f"(confidence: {segment.avg_logprob:.2f})"
                        )
            
            result = ' '.join(text_parts)
            
            if self._logger:
                self._logger.system(f"[Silero VAD] Whisper result: '{result}'")
    ...
    def _speech_processing_loop(self):
        ...
        if self._logger:
            self._logger.system("[Silero VAD] Speech processing loop started")
        ...
        while self._voice_enabled:
            try:
                ...
                if self._logger:
                    self._logger.system(f"[Silero VAD] Got text from queue: '{text}'")
                ...
                if text and len(text) >= 3:
                    if agentname.lower() not in text.lower():
                        ...
                        if self.on_speech_callback:
                            if self._logger:
                                self._logger.system(f"[Silero VAD] Calling callback with: {username}, {text}")
                            self.on_speech_callback(username, text)
                        else:
                            if self._logger:
                                self._logger.error("[Silero VAD] No callback registered!")
                    else:
                        if self._logger:
                            self._logger.system(f"[Silero VAD] Filtered (contains agent name): {text}")
        ...
        if self._logger:
            self._logger.system("[Silero VAD] Speech processing loop stopped")
