...
class SileroVADEngine:
    ...
    def _vad_worker(self, on_speech_callback: Optional[Callable]):
        ...
        while self._is_listening:
            try:
                ...
                if chunks_received % 30 == 0 or (current_time - last_log_time) > 5.0:
                    max_amplitude = np.max(np.abs(audio_chunk)) if len(audio_chunk) > 0 else 0.0
                    if self.logger:
                        self.logger.system(
                            f"[Silero VAD Worker] Chunks: {chunks_received}, "
                ...
                while total_samples >= VAD_CHUNK_SIZE:
                    ...
                    if speech_prob > self.vad_threshold:
                        if not self._in_speech:
                            ...
                            self._speech_buffer = []
                            if self.logger:
                                self.logger.system(f"[Silero VAD] Speech started (prob: {speech_prob:.3f})")
                        ...
                        if self._in_speech:
                            ...
                            if silence_duration >= self.min_silence_duration_ms:
                                ...
                                if speech_duration >= self.min_speech_duration_ms:
                                    ...
                                    if self.logger:
                                        self.logger.system(
                                            f"[Silero VAD] Speech ended ({speech_duration:.0f}ms, "
                                    ...
                                    if self.logger:
                                        self.logger.system(
                                            f"[Silero VAD] Speech too short ({speech_duration:.0f}ms), ignored"
                                        )
                ...
                if (current_time - last_log_time) > 10.0:
                    if self.logger:
                        self.logger.system(
                            f"[Silero VAD Worker] Still running... (chunks: {chunks_received}, "
        ...
        if self.logger:
            self.logger.system(f"[Silero VAD Worker] Stopped (processed {chunks_received} chunks)")
