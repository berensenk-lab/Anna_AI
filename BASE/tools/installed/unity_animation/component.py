...
Dynamic GUI panel for Unity VRM character gesture, emotion, and preset animation control
"""
...
class UnityAnimationComponent:
    ...
    Provides interface for controlling VRM character emotions, gestures, and preset animations
    """
    ...
    def _create_intensity_section(self):
        ...
        ttk.Label(intensity_frame,text="Intensity:",style="TLabel").pack(side=tk.LEFT,padx=(0,5))
        self.intensity_var=tk.DoubleVar(value=0.8)
        intensity_scale=ttk.Scale(intensity_frame,from_=0.0,to=1.0,orient=tk.HORIZONTAL,variable=self.intensity_var,length=200)
        intensity_scale.pack(side=tk.LEFT,fill=tk.X,expand=True,padx=(0,5))
        intensity_label=tk.Label(intensity_frame,textvariable=self.intensity_var,font=("Consolas",9),foreground=self.theme.FG_PRIMARY,background=self.theme.BG_DARKER,width=4)
    ...
    def _execute_custom(self):
        ...
        if command_type=='emotion':
            intensity=self.intensity_var.get()
            self._execute_command('emotion',[value,intensity])
        elif command_type=='preset':
            self._execute_command('preset',[value])
        else:
            self._execute_command('gesture',[value])
    ...
    def _rebuild_emotion_buttons(self):
        ...
        emotion_icons={'happy':'[Happy]','sad':'[Sad]','angry':'[Angry]','surprised':'[Surprised]','neutral':'[Neutral]','relaxed':'[Relaxed]','excited':'[Excited]','confused':'[Confused]'}
        row_frame=None
        for i,emotion in enumerate(self.emotions):
            if i%4==0:
                row_frame=ttk.Frame(self.emotions_container)
                row_frame.pack(fill=tk.X,pady=2)
            icon=emotion_icons.get(emotion,'[Face]')
            text=f"{icon} {emotion.title()}"
            btn=ttk.Button(row_frame,text=text,command=lambda e=emotion:self._execute_emotion(e),width=15)
            btn.pack(side=tk.LEFT,padx=2,expand=True,fill=tk.X)
    ...
    def _rebuild_gesture_buttons(self):
        ...
        self._add_log(f"Gestures updated: {len(self.gestures)} available",'info')

    def _rebuild_loop_buttons(self):
        for btn in self.loop_buttons:
            btn.destroy()
        self.loop_buttons.clear()
        row_frame=None
        for i,loop_anim in enumerate(self.loop_animations):
            if i%4==0:
                row_frame=ttk.Frame(self.loops_container)
                row_frame.pack(fill=tk.X,pady=2)
            display_name=loop_anim.replace('_loop','').replace('_',' ').title()
            text=f"[Loop] {display_name}"
            btn=ttk.Button(row_frame,text=text,command=lambda p=loop_anim:self._execute_preset(p),width=15)
            btn.pack(side=tk.LEFT,padx=2,expand=True,fill=tk.X)
            self.loop_buttons.append(btn)
        self._add_log(f"Loop animations updated: {len(self.loop_animations)} available",'info')
    ...
    def _rebuild_preset_buttons(self):
        ...
        row_frame=None
        for i,preset in enumerate(self.preset_animations):
            if i%4==0:
                row_frame=ttk.Frame(self.presets_container)
                row_frame.pack(fill=tk.X,pady=2)
            text=f"{preset.replace('_',' ').title()}"
            btn=ttk.Button(row_frame,text=text,command=lambda p=preset:self._execute_preset(p),width=15)
            btn.pack(side=tk.LEFT,padx=2,expand=True,fill=tk.X)
            self.preset_buttons.append(btn)
        self._add_log(f"One-shot animations updated: {len(self.preset_animations)} available",'info')
