...
1. **Orient first**: Run `coding.files` at the start of any new coding task to see active files and workspace context.
2. **Fetch before editing**: Always run `coding.fetch` on the target file (or line range) before making any changes. Never edit blind.
3. **Edit with intent**: Use `coding.edit` with a focused instruction and target file whenever possible.
4. **Verify after every change**: Always run `coding.verify` after editing to confirm the change is actually present.
5. **Check health when needed**: Use `coding.status` if command behavior looks stale or disconnected.
...
- If `coding.status` reports unavailable, use `file_system` for folder inspection/tasks instead of stalling
"""
...
### MANDATORY WORKFLOW
1. `coding.files` — get workspace context first
2. `coding.fetch` — read the file (or relevant lines) before editing
3. `coding.edit` — make the change with a focused instruction
4. `coding.verify` — confirm the change was applied
5. `coding.status` — check coding tool state if needed
...
- If you're unsure of a file path, use `coding.files` or ask one focused question — never guess
- If something is ambiguous, ask one focused question before proceeding
- If coding bridge is offline, switch to `file_system` tools for directory/file analysis tasks

### TOOL QUICK REFERENCE
| Goal | Command |
|------|---------|
| Understand project layout | `coding.files []` |
| Read a file or section | `coding.fetch ["file.py", start, end]` |
| Apply an edit | `coding.edit ["instruction", "file.py"]` |
| Confirm change applied | `coding.verify ["file.py", "expected string"]` |
| See open files | `coding.files []` |
| Check tool connection | `coding.status []` |
"""
